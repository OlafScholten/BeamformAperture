...
!    all length and times are measured in meters where  1 ns=0.3 meter=1GHz^{-1}
    ...
    use eventdata, only : PlotBase, FileFitResult, FileShCurrent, FileGrid
    use eventdata, only : Fitting
    Use Systemcommands, only : CreateNewFolder, MakeGLEplots
    implicit none
!    logical first
    INTEGER DATE_T(8),i
...
!
    ...
    real(dp) :: sin_alpha
    NAMELIST /ShPars/ OutFileLabel, PlotBase, test, Fitting, &
    ...
    !
    !pi=2._dp *asin(1._dp)  ! This is the real constant pi
    OutFile='MGMR3D2-v10'
!    first=.true.
    read(*,NML = ShPars)
...
201 format('Grid for \lambda integration, Max number steps=',I4)
   ...
   Endif
   !
   FileFitResult=TRIM(PlotBase)//'FitResult'
   FileShCurrent=TRIM(PlotBase)//'sh_Current'
   FileGrid=TRIM(PlotBase)//'grid'
!
